"""
Demo RAG for training: ChromaDB (in-memory) + local sentence embeddings.

Uses Chroma EphemeralClient (RAM only, no disk). Embeddings from FastEmbed
(ONNX via onnxruntime—no PyTorch), which avoids Python 3.13 + torch wheel issues.
"""
...
# Local embedding model (override with EMBEDDING_MODEL in .env). FastEmbed model id.
# Default: bge-small-en (384-dim ONNX); first run downloads from Hugging Face.
EMBEDDING_MODEL = os.getenv("EMBEDDING_MODEL", "BAAI/bge-small-en-v1.5")

# Tiny fake knowledge base (same text as before; now indexed by vectors)
...
def _embed_normalize(model: TextEmbedding, texts: list[str]) -> list[list[float]]:
    """Batch-embed and L2-normalize rows for cosine distance in Chroma."""
...
def init_vector_store() -> tuple[TextEmbedding, Any]:
    """Load the embedding model and build an in-memory Chroma collection."""
    print(
...
def retrieve(
    ...
    """Embed the query and run a nearest-neighbor search in Chroma."""
...
def run_agent():
    ...
    print(
        "🧠 Vector RAG demo (Chroma in-memory + local embeddings) — "
        "type 'exit' to quit\n"
    )
